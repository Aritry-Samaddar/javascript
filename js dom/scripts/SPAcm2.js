...
const ubtn = document.querySelector('.button')
...
let USERID = ""
// axios.get('http://localhost:3001/empDetails').then((res)=>getAllDetails(res.data)).catch((err)=>console.log(err))
...
userContainer.addEventListener('submit',(e)=>{
    e.preventDefault()
    console.log(USERID)
    if(USERID == "") {
...
contact.addEventListener('click', (e)=>{
    ...
    if(e.target.id === 'edit-btn') {
        ...
        uphone.value=phone
        ucity.value=city
        ubtn.innerHTML = `<button class="btn btn-outline-info" type="submit" id="upd-btn">Update</button>`
        USERID = userid
    }
})
